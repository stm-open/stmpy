...
from .serialize import serialize_object,fmt_hex
from .sign import hash_transaction, HASH_TX_ID, get_stream_from_secret, sign_transaction

__all__ = ('Remote', 'Client', 'StreamError')
...
class ResponseError(StreamError):
...
class TransactionError(ResponseError):
    ...
    higher-level object of our own. For example, the stream server
...
FEE_DEFAULTS = {
    ...
    'fee_base': 10,      # cost of reference transactios in STM
    'load_base': 256,
    'load_factor': 256,
...
class DeferredResponse(object):
    ...
    - Must be resolved with a stream server response, or an exception
      instance.
    - If the response indicates an error, will throw a ResponseError.
    - If resolved with an exception instance, will throw that.
    - Otherwise will resolve the the stream server response.
    """
    def __init__(self):
...
class Client(object):
    ...
    def execute(self, cmd, **data):
        ...
        Possible outcomes are from DeferredResponse.wait(): A stream
    ...
    def add_fee(self, tx, amount=None, cushion='1.2'):
        ...
        """
        if not amount:
            # Note: The stream client uses a design where every transaction
    ...
    def _process_fee_update(self, msg):
        ...
        # The stream client uses these to calculate the STM value of a
    ...
    def request_account_tx(self, account,ledger_index_min,ledger_index_max):
        if(ledger_index_max < 0):
            return self.execute("account_tx", account=account,ledger_index_min=ledger_index_min)
        else:
            return self.execute("account_tx", account=account,ledger_index_min=ledger_index_min,ledger_index_max=ledger_index_max)

    def request_account_lines(self, account):
        return self.execute("account_lines", account=account)['lines']

    def book_offers(self, taker_gets, taker_pays, limit):
        return self.execute("book_offers", taker_gets=taker_gets, taker_pays=taker_pays, limit=limit)

    def account_offers(self, account):
        return self.execute("account_offers", account=account)

    def tx(self, transaction):
        return self.execute("tx", transaction=transaction)
    ...
    def find_path_once(self, source, destination, destination_amount,
        ...
        """Use the ``stream_path_find`` API to find a path"""
        return self.execute(
            "path_find", source_account=source,
...
class Remote(object):
    ...
    def account_tx(self, account,ledger_index_min = -1,ledger_index_max = -1):
        if(ledger_index_max < 0):
            info = self.client.request_account_tx(account,ledger_index_min,-1)
        else:
            info = self.client.request_account_tx(account,ledger_index_min,ledger_index_max)
        return info

    def account_lines(self, account):
        info = self.client.request_account_lines(account)
        return info

    def book_offers(self, taker_gets,taker_pays,limit = 10):
        TakerGets = {}
        TakerPays = {}
        if(taker_gets['currency'] == 'STM'):
            TakerGets = {"currency":"STM"}
        else:
            TakerGets = {"currency":taker_gets['currency'],"issuer":taker_gets['issuer']}
        if(taker_pays['currency'] == 'STM'):
            TakerPays = {"currency":"STM"}
        else:
            TakerPays = {"currency":taker_pays['currency'],"issuer":taker_pays['issuer']}
        info = self.client.book_offers(TakerGets,TakerPays,limit)
        return info

    def account_offers(self, account):
        info = self.client.account_offers(account)
        return info

    def tx(self, transaction):
        info = self.client.tx(transaction)
        return info

    def create_offer(self,taker_gets,taker_pays, account=None):
        if not account:
            if not self.secret:
                raise ValueError(
                    'If you do not provide a account, you need to '
                    'give a secret.')
            account = get_stream_from_secret(self.secret)
        tGets = Amount(taker_gets)
        tPays = Amount(taker_pays)
        if tGets.currency == 'STM':
            TakerGets = str(stm_drops(Decimal(tGets.value)))
        else:
            TakerGets = {
                "currency": tGets.currency,
                "issuer": tGets.issuer,
                "value": str(tGets.value)
            }
        if tPays.currency == 'STM':
            TakerPays = str(stm_drops(Decimal(tPays.value)))
        else:
            TakerPays = {
                "currency": tPays.currency,
                "issuer": tPays.issuer,
                "value": str(tPays.value)
            }
        tx = {
    ...
    def cancel_offer(self,sequence,account=None):
        if not account:
            if not self.secret:
                raise ValueError(
                    'If you do not provide a account, you need to '
        ...
        tx = {
            ...
            "Account" : account,
            "OfferSequence" : Decimal(sequence)
        }
        return self.submit(account, tx)

    def send_payment(self, destination, amount, memo=None, destination_tag=None, account=None,
        flags=None):
        # Parse the amount
        ...
        # to the destination address makes stream pick one.
        if amount.currency != 'STM':
            if not 'issuer' in amount:
                amount['issuer'] = destination
        # For non-stm payments, we need to have a path
        '''
        if amount.currency == 'STM':
            paths = None
        else:
        ...
        if amount.currency != 'STM':
            sendMax = {}
            sendMax["currency"] = amount.currency
            sendMax["issuer"] = amount.issuer
            sendMax["value"] = str(Decimal(amount.value) * 12 / 10)
            tx["SendMax"] = sendMax
        else:
            tx["Amount"] = stm_drops(Decimal(amount.value))
        ...
        if memo is not None:
            if isinstance(memo,six.text_type) or isinstance(memo,six.binary_type):
                memoAdd = {}
                Memos = {}
                tx["Memos"] = []
                memoAdd["MemoType"] = '6D656D6F'
                if six.PY3:
                    memoData = six.binary_type(parse.quote(memo),'utf-8')
                else:
                    memoData = six.binary_type(parse.quote(memo))
                memoAdd["MemoData"] = six.text_type(fmt_hex(memoData))
                Memos["Memo"] = memoAdd
                tx["Memos"].append(Memos)
    ...
    def submit(self, account, tx_json):
        ...
        # This is analog to how stream-client deals with transactions.
        # Plus, see:
